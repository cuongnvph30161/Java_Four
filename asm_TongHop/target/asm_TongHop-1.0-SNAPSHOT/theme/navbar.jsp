...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary p-0">
    <div class="navbar-nav">
        ...
        <c:if test="${not empty sessionScope.username}">
            <a class='nav-item nav-link ${requestURI.contains("/user")?"active":""}' href="user">User Management</a>
        </c:if>

        <c:if test="${not empty sessionScope.username}">
            <a class='nav-item nav-link ${requestURI.contains("/video")?"active":""}' href="video">Video Management</a>
        </c:if>
